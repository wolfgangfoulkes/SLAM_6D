...
public:
	...
	*/
	 ...
	 *									false and using IGeometry::setBillboardModeEnabled instead,
	 *									since that would always keep the geometry upright,
	 *									independent of trackable orientation.
	 * \param	autoScale				True if the plane size should be assigned a height of 100,
	 *									and width of 100*{image width}/{image height}. False if the
	 *									size should be the image width and height (e.g. 640 by 480
	 *									units for a 640x480 image).
	 * \return Pointer to the geometry. Null pointer if not successful.
	 */
