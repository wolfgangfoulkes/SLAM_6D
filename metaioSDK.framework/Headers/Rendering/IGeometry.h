...
/**
 * Rendering options that can be set per geometry
 */
enum ERENDER_OPTION
{
	ERENDEROPTION_BACK_FACE_CULLING = 1,
	ERENDEROPTION_FRONT_FACE_CULLING = 2,
	ERENDEROPTION_ZWRITE = 3,
	ERENDEROPTION_ZTEST = 4,
	ERENDEROPTION_COLOR_MASK = 5,
	ERENDEROPTION_DEBUG_DATA = 6,
	ERENDEROPTION_TEXTURE_FILTERING = 7
};
...
/**
 * Flags determining the type of an animation (either included in model MD2/FBX or added custom animations)
 */
enum EANIMATION_TYPE
{
	EAT_ALL = 0,		///< both types of animation 
	EAT_MODEL_ONLY,		///< only animation included in the model (MD2/FBX)
	EAT_CUSTOM_ONLY		///< only custom animations (added in Creator or via metaioSDK API)
};

/**
 * Flags determining the type of interpolation used in a custom animation
 */
enum EINTERPOLATION_TYPE
{
	EIT_LINEAR = 0		///< currently only linear interpolation
};

/**
 * Structure carrying an animation frame (its index, rotation+translation+scale)
 */
struct AnimationKeyFrame
{
	int 		index;				///< index to store frame number
	Rotation 	rotation;			///< rotation value at this frame
	Vector3d 	translation;		///< translation value at this frame
	Vector3d	scale;				///< scale value at this frame
};

/**
 * Structure carrying a complete custom animation (its interpolation type, vector of keyframes)
 */
struct CustomAnimation
{
	EINTERPOLATION_TYPE type;				///<Interpolation type to use between keyframes (currently only EIT_LINEAR)
	stlcompat::Vector<AnimationKeyFrame> keyframes;  ///<A vector holding all keyframes that constitute the animation to add
...
public:
	...
	/**
	 * Determine whether billboard mode is enabled
	 * \return True if billboard mode is enabled for this geometry, else false
	 * \sa setBillboardModeEnabled
	 */
	virtual bool isBillboardModeEnabled() const = 0;

	/**
	 * Set billboard mode for this geometry, i.e. whether up/right direction in trackable coordinate
	 * system should be aligned with up/right of the screen
	 *
	 * This is not possible for geometries that were already created with the displayAsBillboard
	 * setting (see IMetaioSDK::createGeometryFromImage/createGeometryFromMovie).
	 * As opposed to the displayAsBillboard setting, "billboard mode" will always align the X/Y
	 * axes of the trackable (see
	 * https://dev.metaio.com/sdk/getting-started/terminology-and-coordinate-system/)
	 * with X/Y of the screen (you can still apply additional translation and rotation of course).
	 *
	 * \param	enable	Whether to enable or disable billboard mode
	 * \sa isBillboardModeEnabled
	 */
	virtual void setBillboardModeEnabled(bool enable) = 0;
	 ...
	 * 
	 * With setParentGeometry you can create a scene graph like structure. 
	 * All the parents transformation properties will also be applied to 
	 * the child, this included rotation, translation and scale. Its simpler 
	 * if you do not set scale as a property, but rather scale the actual model 
	 * in your content creation tool accordingly. 
	 * 
	 * \param parent the IGeometry that this one should be attached to.
	 * \sa getParentGeometry
	 */
	virtual void setParentGeometry(metaio::IGeometry* parent) = 0;
	 ...
	 */
	virtual metaio::IGeometry* getParentGeometry() const = 0;
	...
	/**
	 * Set special rendering options of a geometry. This can be used: 
	 ...
	 * \param stopFrame frame number of animation stop (inclusive range).
	 * \param loop If true, the animation is looped, otherwise it is only played once.
	 * \sa setAnimationSpeed
	 */
	virtual void startAnimationRange(unsigned int startFrame, unsigned int stopFrame, bool loop = false) = 0;

	/**
	 * Stops the current animation of the geometry.
	 * \sa startAnimation
	 * \sa startAnimationRange
	 */
	 ...
	 * 
	 * \param animType filter by either custom or model animations
	 * \return list of animation names  
	 * \sa startAnimation
	 */
	virtual stlcompat::Vector<stlcompat::String> getAnimationNames(EANIMATION_TYPE animType = EAT_ALL) const = 0;

	/**
	 * Adds an additional custom animations to this IGeometry
	 * 
	 * \param name Name of the animation to add
	 * \param animation The animation to add (keyframes, interpolation type)
	 * \return true if the animation has been added successfully, false otherwise
	 */
	virtual bool setCustomAnimation(	stlcompat::String name, 
										CustomAnimation animation ) = 0;
	
	/**
	 * Returns a custom animation with given name from this IGeometry (that has been added previously)
	 * 
	 * \param name Name of the animation to retrieve
	 * \return If it could be found in this IGeometry, the animation that has been requested
	 */
	virtual CustomAnimation getCustomAnimation( stlcompat::String name ) = 0;

	/**
	 * Removes a custom animation with given name from this IGeometry
	 * 
	 * \param name Name of the animation to remove
	 * \return True if requested animation could be removed, false otherwise
	 */
	virtual bool removeCustomAnimation( stlcompat::String name ) = 0;

	/**
	 * Set the current playback position of an animation in this IGeometry
	 *
	 * \param frameNr the playback position to set
	 * \param animType the type of animation the playback position should be set
	 */
	virtual void setCurrentFrame(float frameNr, EANIMATION_TYPE animType = EAT_MODEL_ONLY) = 0;
	
	/**
	 * Get the current playback position of an animation in this IGeometry
	 *
	 * \param animType the type of animation which playback position should be retrieved
	 * \return frameNr the current playback position
	 */
	virtual float getCurrentFrame(EANIMATION_TYPE animType = EAT_MODEL_ONLY) = 0;
     ...
     * On iOS these coordinates are scaled according to the screen scale factor.
     * So to get unscaled screen coordinates you have to divide the components
     * by the scale. You can get the scale from the GLKViews contentScaleFactor
	 * property.
     * \return The bounding box of the geometry.
	 */
	 ...
	 *
	 * If the geometry has a parent (setParentGeometry), the geometry will be in the COS of the
	 * highest parent.
	 *
	...
	/// \copydoc setTexture(const stlcompat::String&)
	virtual bool setTexture(const metaio::Path& texturePath) = 0;
	 ...
	 * and right side should have alpha transparency (red channel).
	 *
	 * \param filePath File path of the movie.
	 * \param transparent If true, the movie file will be rendererd as transparent texture.
	 * \sa removeMovieTexture
	 * \return true if successful, false otherwise (e.g. file not found or not loadable).
	 */
	virtual bool setMovieTexture(const metaio::Path& filePath,
