...
/**
 * Minimal wchar_t string implementation
 */
class METAIO_DLL_API StringW
{
public:
	/// Constructor for zero-terminated C string
	 ...
	 * Get a zero-terminated C string.
	 *
	 * The buffer can be expected to be valid during the lifetime of this StringW instance but only
	 * until the next non-const method call.
	 *
	 * \return Zero-terminated C string
	 */
...
public:
	...
	typedef std::vector<T> std_type;	///< typedef for std vectors of type T
...
public:
	...
	/// deletes all elements from this vector
	void deleteAll();
	Vector();
	~Vector();
	void push_back(const T& copyMe);	///< push an object of type T to the end of the vector
