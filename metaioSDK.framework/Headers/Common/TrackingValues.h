...
public:
	...
	double				timeElapsed;				///< Time elapsed (in milliseconds) since last state change of the tracking system
	double				trackingTimeMs;				///< Time (in milliseconds) used for tracking the respective frame
	double				timestampInSeconds;			///< Timestamp (in seconds) of the tracking values,
								///< in the same time system as the camera images.
	 ...
	 */
	static ETRACKING_STATE stringToTrackingState(const stlcompat::String& state, bool* outValidState = 0);
	 ...
	 */
	static stlcompat::String trackingStateToString(ETRACKING_STATE state);
};
}
